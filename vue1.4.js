//实现vue源码的点击事件 v-on 包括@时候不是指令在compileElement中再做一次判断，是@的解析转到v-on中
...
//vue的响应式原理 
//new一个vue的实例化后进行模版的编译和数据的代理，
//数据的代理是指对$data上的每一个值(多层则递归)进行observer的转化，让它能从this.$data.xx转到this.xx
//模版编译的过程先收集他的初始值和他的依赖，收集依赖是靠watcher去收集它依赖$data里的哪些key，通过一个全局的变量dep.target放结果。这样在数据getter的时候就可以知道数据依赖哪些watcher内容，把这部分收集起来后，在setter中通知所有watcher来更新对应的回调函数然后更新模版